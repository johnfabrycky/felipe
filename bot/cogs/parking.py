...
class Parking(commands.Cog):
    ...
    async def claim_spot(
        ...
        start, end, duration = self.service.parse_range(start_day.value, start_time.value, end_day.value,
                                                        end_time.value)
        if duration < timedelta(hours=1) or duration > timedelta(days=7):
            return await interaction.response.send_message("❌ Must be between 1h and 7d.", ephemeral=True)
    ...
    async def parking_help(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            ...
            value=(
                ...
                "   *Note: Claims must be between 1 hours and 7 days long.* "
            ),
            inline=False,
        )
        embed.add_field(
