...
class Parking(commands.Cog):
    ...
    async def parking_status(self, interaction: discord.Interaction):
        ...
        #
        #     # Status Formatting
        #     is_offered_now = any(w["start"] <= now < w["end"] for w in windows)
        #     current_claim = next((c for c in spot_claims if c["start"] <= now < c["end"]), None)
        #     # header = f"🔴 Busy until {current_claim['end'].strftime('%a %I%p')}" if current_claim else "🟢 Available Now"
        #     if current_claim:
        #         header = f"🔴 Busy until {current_claim['end'].strftime('%a %I%p')}"
        #     elif not is_offered_now:
        #         # It's not reserved, but the owner hasn't offered it yet
        #         upcoming_off = next((w for w in windows if w["start"] > now), None)
        #         if upcoming_off:
        #             header = f"🕒 Unavailable (Next: {upcoming_off['start'].strftime('%a %I%p')})"
        #         else:
        #             header = "❌ Not Offered"
        #     else:
        #         # It IS offered now AND there is no claim
        #         upcoming_claim = next((c for c in spot_claims if c["start"] > now), None)
        #         # Check if a claim starts before the current offer window ends
        #         current_window_end = next((w["end"] for w in windows if w["start"] <= now < w["end"]), one_week_later)
        #
        #         # Use the sooner of the two: the next reservation or the end of the offer
        #         limit_time = current_window_end
        #         if upcoming_claim and upcoming_claim['start'] < current_window_end:
        #             limit_time = upcoming_claim['start']
        #
        #         header = f"🟢 Available Now (until {limit_time.strftime('%a %I%p')})"
        #     # else:
